...
(() => {
  ...
  const exportReport = () => {
    ...
    const lines = [];
    lines.push('AGENT SHIELD THREAT REPORT');
    ...
    lines.push('');
    lines.push('-'.repeat(40));
    lines.push('Generated by Agent Shield (https://github.com/texasreaper62/Agent-Shield)');
  ...
  const fetchResults = () => {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      if (!tab.url || tab.url.startsWith('chrome://') || tab.url.startsWith('chrome-extension://') || tab.url.startsWith('about:')) {
        ...
        statusDescription.textContent = 'Agent Shield cannot scan browser internal pages. Navigate to a website to see results.';
        statusCard.classList.remove('status-safe', 'status-caution', 'status-warning', 'status-danger');
  ...
  exportJsonBtn.addEventListener('click', () => {
    ...
    a.href = url;
    a.download = `agent-shield-report-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
