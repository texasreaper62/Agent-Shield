...
- **v13.5** — Detection hardening + security scan remediation: encoding chain detection, SVG injection (Unit 42 real-world attacks), structured data injection. Crescendo attack defense in cross-turn detector. Persistent memory poisoning defense. Security hardening: sidecar auth/rate-limiting, dashboard WebSocket auth, GitHub webhook enforcement, document scanner size limits, audit log sanitization
- **v13.6** — Performance leap + hardening: long-benign fast path (15.7ms → 112μs p99, 140x speedup), plugin VM sandbox with signature verification, Express middleware body limits, multi-tenant auth validation, ReDoS audit (0 risky patterns), pattern quality audit (120 active / 177 dead), honest latency benchmark (p50/p95/p99)
- **v14.0** — Platform parity: OpenAI Agents SDK integration (April 2026 release), CrewAI/Google ADK/MS Agent Framework wrappers, Rust NAPI scanner bridge, Python+Go SDK sync (141→179 patterns), GETTING_STARTED.md, Microsoft Agent Governance Toolkit parity audit
- **v13.6** — Performance leap + hardening: long-benign fast path (15.7ms → 112μs p99, 140x speedup), plugin VM sandbox with signature verification, Express middleware body limits, multi-tenant auth validation, ReDoS audit (0 risky patterns), pattern quality audit (120 active / 177 dead), honest latency benchmark (p50/p95/p99)
